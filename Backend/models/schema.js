...
const registrationSchema = Joi.object({
    ...
    userPhone: Joi.string().required().regex(/^[0-9]{10}$/).messages({ 'string.pattern.base': `Phone number must have 10 digits.` }).required()
});
...
const bookingsSchema = Joi.object({
    ...
    person: Joi.number().required().min(1).max(10000).messages({ 'reason': "Thala for reason!" }),
...
module.exports = {signUpSchema,updateUserSchema, registrationSchema, bookingsSchema, newSectionSchema,eventSchema,editEventSchema,testimonialSchema,editSchema,workshopSchema};